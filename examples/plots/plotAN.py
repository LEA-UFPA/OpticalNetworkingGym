...
def gerar_grafico_probabilidade_bloqueio(
    ...
    **kwargs
    ...
    """
    print(f"A iniciar a geração do gráfico de probabilidade de bloqueio para {topology_name}...")

    # CONFIGURAÇÃO
    # topology_name vem dos argumentos

    base_results_dir = results_dir
    band_folders = ["BandaC+L+S", "BandaC+S", "BandaL+S", "BandaC", "BandaL", "BandaS"]
    markers = ("o", ">", "s", "D", "^", "v")

    plt.figure(figsize=(5, 4))

    # Regex ajustado para ser mais flexível com o nome da topologia
    escaped_topo = re.escape(topology_name)
    # Tenta capturar load_results_{topo}_{heuristic}_{band}_{load}.csv ou load_results_{topo}_{load}.csv
    # O script original esperava load_results_[\w\-]+_(\d+)\.csv
    # Vamos manter a lógica original mas usando o topology_name se possível, ou genérico se preferir.
    # O original usava [\w\-]+ que pegava qualquer coisa.
    # Vamos adaptar para filtrar pela topologia se fornecida.
    
    # load_pattern = re.compile(r"load_results_[\w\-]+_(\d+)\.csv") 
    # Melhor usar o topology_name para evitar pegar arquivos de outras simulações se misturados
    load_pattern = re.compile(rf"load_results_{escaped_topo}.*_(\d+(?:\.\d+)?)\.csv")
    ...
    for idx, band_folder in enumerate(band_folders):
        ...
        for f in files:
            ...
            if match:
                load = float(match.group(1)) # Alterado para float para suportar cargas não inteiras
                full_file_path = os.path.join(full_band_path, f)
                try:
                    data_load = pd.read_csv(full_file_path, comment='#')

                    # Verifica se a coluna existe
                    if 'episode_service_blocking_rate' not in data_load.columns:
                         print(f"Aviso: Coluna 'episode_service_blocking_rate' não encontrada em {f}")
                         continue
