...
def starmap_helper(args):
    ...
    return run_environment_with_monitoring(*args)
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        '-t', '--topology_file',
        type=str,
        default='nsfnet_chen.txt',  # Default topology from the paper
        help='Topology file name (e.g., nsfnet_chen.txt)'
    )
    parser.add_argument(
        '-e', '--num_episodes',
        type=int,
    ...
    parser.add_argument(
        '-hi', '--heuristic_index',
        type=int,
        default=6,  # Shortest Path, used as baseline 'SP' in the paper
        choices=[1, 2, 3, 4, 5, 6],
        help='Índice da heurística (1: First Fit/Shortest Path, 2: Highest SNR, 3: Lowest Fragmentation, 4: Load Balancing, 5: Prioridade Banda C, 6: Multibanda Best Band)'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '-l', '--load',
        type=float,
        default=2500.0,  # Starting load from the paper's analysis
        help='Carga da simulação (default: 2500.0)'
    )
    parser.add_argument(
        '-p', '--power',
        type=float,
        default=-3.66,  # C-band launch power from the paper
        help='Potência de lançamento em dBm (default: -3.66)'
    )
...
def main():
    ...
    # Band specs from paper Table II. Note: Launch power is set globally via args, not per-band here.
    band_specs_options = {
    ...
    for band_name in args.bands:
        print(f"\n--- Preparando simulação para banda: {band_name} ---")
        ...
        env_args.append((
            ...
            (100, 500),
            0,
            f"service_logs/load_services_{band_name}_{args.load}.csv" if args.monitor_file_name else "",
            False,
            False,
            0,
            False,
    ...
    print(f"Iniciando {len(env_args)} simulações em {args.threads} threads...")
    ...
    if args.threads > 1:
        ...
        for arg in tqdm(env_args, desc="Simulando Cargas/Bandas"):
            run_environment_with_monitoring(*arg)
