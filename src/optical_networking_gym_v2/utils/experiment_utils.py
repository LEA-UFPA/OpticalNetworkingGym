...
def select_informative_first_fit_policy(env: object, info: dict[str, object]) -> int:  # noqa: ARG001
    """Política first-fit informativa.

    Diferente de :func:`select_masked_first_fit_policy`, esta versão não usa a
    máscara binária pré-calculada. Em vez disso, inspeciona diretamente os arrays
    ``resource_valid_starts`` e ``qot_valid_starts`` do simulador para retornar
    uma ação que permite ao simulador distinguir corretamente:

    * ``BLOCKED_RESOURCES`` — sem espectro disponível.
    * ``BLOCKED_QOT``       — espectro disponível, mas OSNR insuficiente.
    * ``accepted``          — alocação realizada com sucesso.
    """
    from optical_networking_gym_v2.heuristics.runtime_heuristics import (
        select_first_fit_informative_action,
    )
    return int(select_first_fit_informative_action(env))
...
__all__ = [
    ...
    "float_mean",
    "float_std",
    "select_informative_first_fit_policy",
